...
const MyGroups = {
     ...
     */
    resolveCurrentUserUplineAccess(profile = this.currentMemberProfile) {
        const uid = String(window.currentUser?.uid || '').trim();
        if (!uid) return { valid: false, source: 'no_user', groupId: null };

        const candidateProfile = (profile && typeof profile === 'object') ? profile : {};
        const downlineGroupIds = new Set(
            (Array.isArray(this.downlineGroups) ? this.downlineGroups : [])
                .map((group) => String(group?.id || '').trim())
                .filter(Boolean)
        );

        // Meeting gating should prefer real upline hints over false-negative cache gaps.
        const primaryLoadedUplineId = String(this.uplineGroup?.id || '').trim();
        const primaryLoadedLeaderId = String(this.uplineGroup?.leaderId || '').trim();
        if (primaryLoadedUplineId && primaryLoadedLeaderId && primaryLoadedLeaderId !== uid) {
            return {
                valid: true,
                source: 'loaded_upline_group',
                groupId: primaryLoadedUplineId
            };
        }

        const profileUplineGroupId = String(candidateProfile?.uplineGroupId || '').trim();
        const profileGroupId = String(candidateProfile?.groupId || '').trim();
        const normalizedRole = String(candidateProfile?.groupRole || '').toLowerCase().trim();

        const nonSelfVisibleGroups = [
            ...(Array.isArray(this.guestGroups) ? this.guestGroups : [])
        ].filter((group) => {
            const groupId = String(group?.id || '').trim();
            const leaderId = String(group?.leaderId || '').trim();
            return groupId && !downlineGroupIds.has(groupId) && leaderId && leaderId !== uid;
        });

        const recoveredMemberGroup = nonSelfVisibleGroups.find((group) => {
            const groupId = String(group?.id || '').trim();
            return this.isUserMemberInGroupData(group, uid) || profileUplineGroupId === groupId;
        });
        if (recoveredMemberGroup) {
            return {
                valid: true,
                source: 'visible_non_self_group',
                groupId: String(recoveredMemberGroup.id || '').trim()
            };
        }

        if (profileUplineGroupId && !downlineGroupIds.has(profileUplineGroupId)) {
            return {
                valid: true,
                source: 'profile_upline_pointer',
                groupId: profileUplineGroupId
            };
        }

        if (normalizedRole === 'member' && profileGroupId && !downlineGroupIds.has(profileGroupId)) {
            return {
                valid: true,
                source: 'profile_member_group_pointer',
                groupId: profileGroupId
            };
        }

        return { valid: false, source: 'missing_upline', groupId: null };
    },

    currentUserHasValidUpline(profile = this.currentMemberProfile) {
        return this.resolveCurrentUserUplineAccess(profile).valid;
    },
